...
export default function Orders() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 px-3 py-4 sm:px-4 md:px-8">
      {/* Header */}
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          <div className="inline-flex items-center gap-2 rounded-full bg-amber-500/10 px-3 py-1 ring-1 ring-amber-400/40">
            ...
            <span className="text-[10px] sm:text-[11px] uppercase tracking-[0.18em] text-amber-200">
              Admin · Orders
            </span>
          </div>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={fetchOrders}
            disabled={loading}
            className="inline-flex items-center gap-2 rounded-full border border-slate-600 bg-slate-900/80 px-3 py-1.5 text-xs sm:text-sm font-medium text-slate-100 shadow-sm hover:border-slate-400 hover:bg-slate-800/90 disabled:opacity-60 disabled:cursor-not-allowed transition-all"
          >
            ...
            <div className="block md:hidden divide-y divide-slate-800">
              {orders.map((order, idx) => {
                ...
                return (
                  <div
                    key={order._id}
                    className="px-4 py-4 flex flex-col gap-3 hover:bg-slate-900/60 transition-colors"
                  >
                    {/* Top row: order + status */}
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex flex-col gap-1">
                        <span className="font-mono text-[11px] text-amber-200 break-all">
                      ...
                      <span
                        className={`inline-flex items-center gap-1 rounded-full px-2 py-[3px] text-[11px] ${statusClass}`}
                      >
                    ...
                    {order.items?.length > 0 && (
                      <div className="rounded-lg border border-slate-800 bg-slate-900/60 p-2">
                        <span className="block mb-1 text-[11px] font-medium text-slate-300">
                          Items
                        </span>
                        <div className="space-y-1">
                    ...
                    {/* Payment + date + status select + actions */}
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <div className="flex-1 min-w-[140px] flex flex-col items-end gap-2">
                        <span className="text-[10px] text-slate-400 text-right">
                          {displayDate}
                        </span>
                        <select
                          className="w-full text-[11px] rounded-md border border-slate-700 bg-slate-900/80 px-2 py-1 text-slate-100 focus:border-sky-400 focus:outline-none focus:ring-0 disabled:opacity-60"
                        ...
                        <button
                          className="text-[11px] text-rose-300 hover:text-rose-200 hover:underline disabled:opacity-60"
